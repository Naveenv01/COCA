...
async function insertMongoDb(textData: string, uploadedFile: File) {
  const sentences = textData.split(/(?<=[.!?])\s+/).filter(sentence => sentence.trim().length > 0)
  ...
  const sentencesWithMetadata = sentences.map(sentence => ({
    text:sentence.replace(/[^a-zA-Z\s]/g, '').replace(/\s+/g, ' ').trim(),
    fileId: insertedId,
    fileName: uploadedFile.name
  }))
console.log(sentencesWithMetadata)
